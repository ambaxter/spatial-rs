...
#[allow(dead_code)]
#[derive(Debug)]
pub struct Leaf<P, D, T>
    where D: ArrayLength<P> + ArrayLength<(P, P)>
{
    pub shape: Shapes<P, D>,
    pub item: T,
}

impl<P, D, T> Shape<P, D> for Leaf<P, D, T>
...
#[allow(dead_code)]
#[derive(Debug)]
pub enum LevelNode<P, D, T>
    where D: ArrayLength<P> + ArrayLength<(P, P)>
{
    Level {
        mbr: Rect<P, D>,
        children: Vec<LevelNode<P, D, T>>,
    },
    Leaves {
        mbr: Rect<P, D>,
        children: Vec<Leaf<P, D, T>>,
    },
}

#[allow(dead_code)]
impl<P, D, T> LevelNode<P, D, T>
...
impl<P, D, T> Shape<P, D> for LevelNode<P, D, T>
    where P: Float + Signed + Bounded + MulAssign + AddAssign + ToPrimitive + FromPrimitive + Copy + Debug + Default,
          D: ArrayLength<P> + ArrayLength<(P,P)> {
...
#[allow(dead_code)]
#[derive(Debug)]
pub enum Query<P, D, T>
    where D: ArrayLength<P> + ArrayLength<(P, P)>
{
    Contains(Rect<P, D>, PhantomData<T>),
    Overlaps(Rect<P, D>, PhantomData<T>),
}

#[allow(dead_code)]
impl<P, D, T> Query<P, D, T>
    where P: Float + Signed + Bounded + MulAssign + AddAssign + ToPrimitive + FromPrimitive + Copy + Debug + Default,
          D: ArrayLength<P> + ArrayLength<(P,P)> {

    pub fn accept_leaf(&self, leaf: &Leaf<P, D, T>) -> bool {
        match self {
            &Query::Contains(ref query, _) => leaf.shape.contained_by_rect(&query),
            &Query::Overlaps(ref query, _) => leaf.shape.overlapped_by_rect(&query),
        }
    }

    pub fn accept_level(&self, level: &LevelNode<P, D, T>) -> bool {
        match self {
            &Query::Contains(ref query, _) => level.mbr().overlapped_by_rect(&query),
            &Query::Overlaps(ref query, _) => level.mbr().overlapped_by_rect(&query),
        }
    }
}

#[allow(dead_code)]
pub struct Tree<P, D, I, R, T>
    where D: ArrayLength<P> + ArrayLength<(P, P)>,
          I: IndexInsert<P, D, T>,
          R: IndexRemove<P, D, T, I>
{
    dim: usize,
    insert_index: I,
    remove_index: R,
          ...
          R: IndexRemove<P, D, T, I>,
{
    ...
    pub fn insert(&mut self, shape: Shapes<P, D>, item: T) {
        self.root = self.insert_index.insert_into_root(self.root.take(), Leaf{shape: shape, item: item});
        self.len += 1;
    }

    pub fn remove(&mut self, query: Query<P, D, T>) -> Vec<Leaf<P, D, T>> {
        return self.retain(query, |_| false);
    }

    pub fn retain<F: FnMut(&T) -> bool>(&mut self, query: Query<P, D, T>, mut f: F) -> Vec<Leaf<P, D, T>> {
        let (new_root, removed) =
            self.remove_index.remove_from_root(self.root.take(), &self.insert_index, query);
    ...
    pub fn iter(&self) -> Iter<P, D, T> {
        Iter::new(Query::Overlaps(Rect::max(), PhantomData), self.root.as_ref())
    }

    pub fn iter_mut(&mut self) -> IterMut<P, D, T> {
        IterMut::new(Query::Overlaps(Rect::max(), PhantomData), self.root.as_mut())
    }

    pub fn query(&self, query: Query<P, D, T>) -> Iter<P, D, T> {
        Iter::new(query, self.root.as_ref())
    }

    pub fn query_mut(&mut self, query: Query<P, D, T>) -> IterMut<P, D, T> {
        IterMut::new(query, self.root.as_mut())
    }
}

#[allow(dead_code)]
pub struct Iter<'tree, P, D, T>
    where P: 'tree,
          D: ArrayLength<P> + ArrayLength<(P, P)> + 'tree,
          T: 'tree
{
    query: Rc<Query<P, D, T>>,
    level_iter: LevelIter<'tree, P, D, T>,
    leaf_iter: Option<SliceIter<'tree, Leaf<P, D, T>>>,
    finished: bool,
}

#[allow(dead_code)]
impl<'tree, P, D, T> Iter<'tree, P, D, T>
    where P: Float + Signed + Bounded + MulAssign + AddAssign + ToPrimitive + FromPrimitive + Copy + Debug + Default + 'tree,
          D: ArrayLength<P> + ArrayLength<(P,P)> + 'tree,
          T: 'tree
{
    pub fn new(query: Query<P, D, T>, root: Option<&'tree LevelNode<P, D, T>>) -> Iter<'tree, P, D, T> {
    ...
    fn next_leaf(&mut self, mut iter: SliceIter<'tree, Leaf<P, D, T>>) -> Option<(&'tree Shapes<P, D>, &'tree T)> {
...
impl<'tree, P, D, T> Iterator for Iter<'tree, P, D, T>
    where P: Float + Signed + Bounded + MulAssign + AddAssign + ToPrimitive + FromPrimitive + Copy + Debug + Default + 'tree,
          D: ArrayLength<P> + ArrayLength<(P,P)> + 'tree,
          T: 'tree
{
    type Item = (&'tree Shapes<P, D>, &'tree T);

    fn next(&mut self) -> Option<(&'tree Shapes<P, D>, &'tree T)> {
        if self.finished {
            return None;
        }
...
#[allow(dead_code)]
pub struct LevelIter<'tree, P, D, T>
    where P: 'tree,
          D: ArrayLength<P> + ArrayLength<(P, P)> + 'tree,
          T: 'tree
{
    query: Rc<Query<P, D, T>>,
    root: Option<&'tree LevelNode<P, D, T>>,
    level_stack: Vec<SliceIter<'tree, LevelNode<P, D, T>>>,
    finished: bool,
}


#[allow(dead_code)]
impl<'tree, P, D, T> LevelIter<'tree, P, D, T>
    where P: Float + Signed + Bounded + MulAssign + AddAssign + ToPrimitive + FromPrimitive + Copy + Debug + Default + 'tree,
          D: ArrayLength<P> + ArrayLength<(P,P)> + 'tree,
          T: 'tree {

    pub fn new(query: Rc<Query<P, D, T>>, root: Option<&'tree LevelNode<P, D, T>>) -> LevelIter<'tree, P, D, T> {
    ...
    fn next_leaves(&mut self, mut m_iter: SliceIter<'tree, LevelNode<P, D, T>>) -> Option<SliceIter<'tree, Leaf<P, D, T>>> {
...
impl<'tree, P, D, T> Iterator for LevelIter<'tree, P, D, T>
    where P: Float + Signed + Bounded + MulAssign + AddAssign + ToPrimitive + FromPrimitive + Copy + Debug + Default + 'tree,
          D: ArrayLength<P> + ArrayLength<(P,P)> + 'tree,
          T: 'tree {
    type Item = SliceIter<'tree, Leaf<P, D, T>>;

    fn next(&mut self) -> Option<SliceIter<'tree, Leaf<P, D, T>>> {
        if self.finished {
            return None;
        }
...
#[allow(dead_code)]
pub struct IterMut<'tree, P, D, T>
    where P: 'tree,
          D: ArrayLength<P> + ArrayLength<(P, P)> + 'tree,
          T: 'tree
{
    query: Rc<Query<P, D, T>>,
    level_iter: LevelIterMut<'tree, P, D, T>,
    leaf_iter: Option<SliceIterMut<'tree, Leaf<P, D, T>>>,
    finished: bool,
}

#[allow(dead_code)]
impl<'tree, P, D, T> IterMut<'tree, P, D, T>
    where P: Float + Signed + Bounded + MulAssign + AddAssign + ToPrimitive + FromPrimitive + Copy + Debug + Default + 'tree,
          D: ArrayLength<P> + ArrayLength<(P,P)> + 'tree,
          T: 'tree
{
    pub fn new(query: Query<P, D, T>, root: Option<&'tree mut LevelNode<P, D, T>>) -> IterMut<'tree, P, D, T> {
        let rc_query = Rc::new(query);
    ...
    fn next_leaf(&mut self, mut iter: SliceIterMut<'tree, Leaf<P, D, T>>) -> Option<(&'tree Shapes<P, D>, &'tree mut T)> {
        while let Some(ref mut leaf) = iter.next() {
                ...
                let mut cast_leaf = unsafe {
                    let cast_leaf: *mut Leaf<P, D, T> = *leaf;
                    &mut *cast_leaf
                };
...
impl<'tree, P, D, T> Iterator for IterMut<'tree, P, D, T>
    where P: Float + Signed + Bounded + MulAssign + AddAssign + ToPrimitive + FromPrimitive + Copy + Debug + Default + 'tree,
          D: ArrayLength<P> + ArrayLength<(P,P)> + 'tree,
          T: 'tree
{
    type Item = (&'tree Shapes<P, D>, &'tree mut T);

    fn next(&mut self) -> Option<(&'tree Shapes<P, D>, &'tree mut T)> {
        if self.finished {
            return None;
        }
        if self.leaf_iter.is_none() {
...
#[allow(dead_code)]
pub struct LevelIterMut<'tree, P, D, T>
    where P: 'tree,
          D: ArrayLength<P> + ArrayLength<(P, P)> + 'tree,
          T: 'tree
{
    query: Rc<Query<P, D, T>>,
    root: Option<&'tree mut LevelNode<P, D, T>>,
    level_stack: Vec<SliceIterMut<'tree, LevelNode<P, D, T>>>,
    finished: bool,
}

#[allow(dead_code)]
impl<'tree, P, D, T> LevelIterMut<'tree, P, D, T>
    where P: Float + Signed + Bounded + MulAssign + AddAssign + ToPrimitive + FromPrimitive + Copy + Debug + Default +'tree,
          D: ArrayLength<P> + ArrayLength<(P,P)> + 'tree,
          T: 'tree {

    pub fn new(query: Rc<Query<P, D, T>>, root: Option<&'tree mut LevelNode<P, D, T>>) -> LevelIterMut<'tree, P, D, T> {
        if root.is_none() || !query.accept_level(root.as_ref().unwrap()) {
...
// We're not modifying the tree structure, just the leaf nodes, so this should be ok
    unsafe fn get_root_node(&mut self) -> &'tree mut LevelNode<P, D, T> {
        let root: *mut Option<&'tree mut LevelNode<P, D, T>> = &mut self.root;
        *(*root).as_mut().unwrap()
    }

fn next_leaves(&mut self, mut m_iter: SliceIterMut<'tree, LevelNode<P, D, T>>) -> Option<SliceIterMut<'tree, Leaf<P, D, T>>> {
        let mut iter_node = m_iter.next();
        while let Some(node) = iter_node {
            if !self.query.accept_level(node) {
                iter_node = m_iter.next();
...
impl<'tree, P, D, T> Iterator for LevelIterMut<'tree, P, D, T>
    where P: Float + Signed + Bounded + MulAssign + AddAssign + ToPrimitive + FromPrimitive + Copy + Debug + Default + 'tree,
          D: ArrayLength<P> + ArrayLength<(P,P)> + 'tree,
          T: 'tree {
    type Item = SliceIterMut<'tree, Leaf<P, D, T>>;

    fn next(&mut self) -> Option<SliceIterMut<'tree, Leaf<P, D, T>>> {
        if self.finished {
            return None;
        }
        if self.level_stack.is_empty() {
